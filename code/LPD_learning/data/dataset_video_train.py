...
class DataLoaderBRSVideo(Dataset):
    def __init__(self, root_dir, num_frames=12, patch_size=None, noise=None, is_train=True):
        super(DataLoaderBRSVideo, self).__init__()
        self.num_frames = num_frames
        self.turb_list = []
        for v in os.listdir(root_dir):
            self.turb_list.append(os.path.join(root_dir, v))

        self.ps = patch_size
        self.sizex = len(self.turb_list)  # get the size of target
        self.train = is_train
        self.noise = noise
    ...
    def _fetch_chunk_train(self, idx):
        ...
        if total_frames < self.num_frames:
            print('no enough frame in video ' + self.turb_list[idx])
        start_frame_id = (total_frames-self.num_frames) // 2
    ...
    def __getitem__(self, index):
        index_ = index % self.sizex
        try:
            turb_imgs = self._fetch_chunk_train(index_)
        except:
            print(self.turb_list[index_])
            turb_imgs = self._fetch_chunk_train((index+1) % self.sizex)
        return torch.stack(turb_imgs, dim=0), "test"