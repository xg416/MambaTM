...
class DataLoaderTurbVideo(Dataset):
    def __init__(self, data_path, info_path, turb=True, tilt=False, blur=False, blur2=False, 
                        level='all', num_frames=12, patch_size=None, noise=None, is_train=True, train_prefix='train'):
        ...
        if is_train:
            ...
            if blur2:
                self.blur_dir = os.path.join(data_path, train_prefix + '_blur2')
            else:
                self.blur_dir = os.path.join(data_path, train_prefix + '_blur')
            self.turb_dir = os.path.join(data_path, train_prefix + '_turb')
            self.tilt_dir = os.path.join(data_path, train_prefix + '_tilt')
            self.gt_dir = os.path.join(data_path, train_prefix + '_gt')
        else:
            if blur2:
    ...
    def __getitem__(self, index):
        ...
        if self.require_turb and (not self.require_tilt) and (not self.require_blur):
            return torch.stack(loaded_imgs['turb'], dim=0), torch.stack(loaded_imgs['gt'], dim=0), self.train_prefix
...
class DataLoaderTurbVideoSemi(Dataset):
    def __init__(self, data_path, info_path, turb=True, tilt=False, blur=False,
                 level='all', num_frames=12, patch_size=None, noise=None, is_train=True, train_prefix='train'):
        super(DataLoaderTurbVideoSemi, self).__init__()
        self.num_frames = num_frames
        self.require_turb = turb
        self.require_tilt = tilt
        self.require_blur = blur
        self.train_prefix = train_prefix
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            ...
            loaded_imgs = self._fetch_chunk_val(index_)
        if not self.require_turb and (not self.require_tilt) and (not self.require_blur):
            return torch.stack(loaded_imgs['gt'], dim=0), self.train_prefix
        ...
        if (not self.require_turb) and self.require_tilt and (not self.require_blur):
            return torch.stack(loaded_imgs['gt'], dim=0), torch.stack(loaded_imgs['tilt'], dim=0), self.train_prefix
