...
def get_args():
    ...
    parser.add_argument('--run_name', type=str, default='MambaTM3Ref', help='name of this running')
...
def validate(args, model, val_loader, criterion, criterion_lpips, TS, iter_count, im_save_freq, im_save_path, device, level):
        ...
        for s, data in enumerate(val_loader):
            ...
            with torch.no_grad():
                ...
                loss = criterion(output, target) + 0.02 * loss_lpips
...
def main():
    ...
    for epoch in range(1000000):
        for data in train_loader:
            ...
            loss_lpips = criterion_lpips(output.flatten(0,1)*2-1, target.flatten(0,1)*2-1).mean()
            loss = criterion_char(output, target) + 0.02 * loss_lpips
            loss_all = loss + args.rt_weight * (loss_returb + 0.0002 * kld)
