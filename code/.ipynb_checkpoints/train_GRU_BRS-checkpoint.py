...
def get_args():
    ...
    parser.add_argument('--iters', type=int, default=200000, help='Number of epochs')
    ...
    parser.add_argument('--num_frames', type=int, default=20, help='number of frames for the model')
    parser.add_argument('--num_workers', type=int, default=8, help='number of workers in dataloader')
    parser.add_argument('--train_path', type=str, default='/home/zhan3275/data/BRS_syn/lmdb/train_lmdb/', help='path of training imgs')
    parser.add_argument('--train_info', type=str, default='/home/zhan3275/data/BRS_syn/lmdb/train_lmdb/train_info.json', help='info of training imgs')
    parser.add_argument('--val_path', type=str, default='/home/zhan3275/data/BRS_syn/lmdb/test_lmdb/', help='path of validation imgs')
    parser.add_argument('--val_info', type=str, default='/home/zhan3275/data/BRS_syn/lmdb/test_lmdb/test_info.json', help='info of testing imgs')   
    parser.add_argument('--load', '-f', type=str, default=False, help='Load model from a .pth file')
    parser.add_argument('--log_path', type=str, default='/home/zhan3275/data/log_rnn/BRS/', help='path to save logging files and images')
    parser.add_argument('--task', type=str, default='turb', help='choose turb or blur or both')
    parser.add_argument('--run_name', type=str, default='BRS_RTM_GRU', help='name of this running')
    parser.add_argument('--start_over', action='store_true')

    parser.add_argument('--model', type=str, default='RTM_GRU', help='type of model to construct')
...
def main():
    ...
    train_loader = DataLoader(dataset=train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, drop_last=True, pin_memory=True, prefetch_factor=3)
    ...
    val_loader_weak = DataLoader(dataset=val_dataset_weak, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, drop_last=False, pin_memory=True, prefetch_factor=1)

    val_dataset_medium = DataLoaderTurbVideo(args.val_path, args.val_info, turb=True, tilt=False, blur=False, level='medium', \
                                    num_frames=args.num_frames, patch_size=args.patch_size, noise=0.0001, is_train=False)
    val_loader_medium = DataLoader(dataset=val_dataset_medium, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, drop_last=False, pin_memory=True, prefetch_factor=1)

    val_dataset_strong = DataLoaderTurbVideo(args.val_path, args.val_info, turb=True, tilt=False, blur=False, level='strong', \
                                    num_frames=args.num_frames, patch_size=args.patch_size, noise=0.0001, is_train=False)
    val_loader_strong = DataLoader(dataset=val_dataset_strong, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, drop_last=False, pin_memory=True, prefetch_factor=1)
