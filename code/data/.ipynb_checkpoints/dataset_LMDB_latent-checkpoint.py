...
class DataLoaderTurbVideo(Dataset):
    def __init__(self, data_path, info_path, turb=True, tilt=False, level='all', num_frames=12, patch_size=None, noise=None, is_train=True):
        super(DataLoaderTurbVideo, self).__init__()
        self.num_frames = num_frames
        self.require_turb = turb
        self.require_tilt = tilt
        self.require_latent = True
        ...
        if is_train:
            ...
            self.latent_dir = os.path.join(data_path, 'train_latent')
        else:
            ...
            self.latent_dir = os.path.join(data_path, 'test_latent')
        ...
        if self.require_latent:
            if is_train:
                self.env['latent'] = lmdb.open(self.latent_dir, readonly=True, lock=False, readahead=True, map_size=3*1099511627776)
            else:
                self.env['latent'] = lmdb.open(self.latent_dir, readonly=True, lock=False, readahead=True, map_size=1099511627776)
            self.txn['latent'] = self.env['latent'].begin()
    ...
    def _fetch_chunk_train(self, idx):
        ...
        for fi in range(start_frame_id, start_frame_id+self.num_frames):
            ...
            for modality in self.required:
                enc_key = self.txn[modality].get(key.encode())
                load_img = np.frombuffer(enc_key, dtype='uint8')
                imgs[modality].append(load_img.reshape(h,w,3))
            enc_key = self.txn["latent"].get(key.encode())
            load_img = np.frombuffer(enc_key, dtype='float32')
            latent.append(load_img.reshape(2, h, w))
        ...
        for modality in self.required:
            imgs[modality] = [Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB)) for img in imgs[modality]]
        latent = [TF.to_tensor(img).permute() for img in latent]
        ...
        if padw!=0 or padh!=0:
            for modality in self.required:
                imgs[modality] = [TF.pad(img, (0,0,padw,padh), padding_mode='reflect') for img in imgs[modality]]
        latent = [TF.pad(img, (0,0,padw,padh), padding_mode='reflect') for img in latent]
        ...
        crop_size = int(ps * enlarge_factor)
        crop_size = min(hh, ww, crop_size)
        # hcro = int(crop_size * random.uniform(1.1, 0.9))
        # wcro = int(crop_size * random.uniform(1.1, 0.9))
        hcro = min(crop_size, hh)
        wcro = min(crop_size, ww)
        rr   = random.randint(0, hh-hcro)
        cc   = random.randint(0, ww-wcro)
        
        # Crop patch
        for modality in self.required:
            imgs[modality] = [TF.resize(img.crop((rr, cc, rr+hcro, cc+wcro)), (ps, ps)) for img in imgs[modality]]  
            imgs[modality] = [TF.to_tensor(img) for img in imgs[modality]]
        latent = [TF.resize(img.crop((rr, cc, rr+hcro, cc+wcro)), (ps, ps)) for img in latent]  
    ...
    def __getitem__(self, index):
        ...
        if self.require_tilt:
            return torch.stack(loaded_imgs['turb'], dim=0), torch.stack(loaded_imgs['gt'], dim=0), \
                    torch.stack(loaded_imgs['latent'], dim=0), torch.stack(loaded_imgs['tilt'], dim=0)
        else:
            return torch.stack(loaded_imgs['turb'], dim=0), torch.stack(loaded_imgs['gt'], dim=0), torch.stack(loaded_imgs['latent'], dim=0)
