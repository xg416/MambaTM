...
class DataLoaderTurbVideo(Dataset):
    ...
    def _fetch_chunk_train(self, idx):
        ...
        # Crop patch
        blur_imgs = [TF.resize(img.crop((cc, rr, cc+wcro, rr+hcro)), (ps, ps), antialias=True) for img in blur_imgs]
        turb_imgs = [TF.resize(img.crop((cc, rr, cc+wcro, rr+hcro)), (ps, ps), antialias=True) for img in turb_imgs]
        tar_imgs  = [TF.resize(img.crop((cc, rr, cc+wcro, rr+hcro)), (ps, ps), antialias=True) for img in tar_imgs]
...
class DataLoaderTurbVideoTest(Dataset):
    def __init__(self, root_dir, result_dir, max_frame=120, patch_unit=8, smallest_patch=256):
...
class DataLoaderTurbImage(Dataset):
    ...
    def __init__(self, rgb_dir, num_frames=12, total_frames=60, im_size=None, noise=None, other_mod='blur', is_train=True, prefix="{:d}.png"):
    ...
    def _fetch_chunk_val(self, seq_path, frame_idx):
        ps = self.ps
        other_img_list = [os.path.join(seq_path, self.other_mod, self.prefix.format(n)) for n in frame_idx]
        turb_img_list = [os.path.join(seq_path, 'turb', self.prefix.format(n)) for n in frame_idx]
    ...
    def _fetch_chunk_train(self, seq_path, frame_idx):
        ...
        # Crop patch
        other_imgs = [TF.resize(img[:, rr:rr+hcro, cc:cc+wcro], (ps, ps), antialias=True) for img in other_imgs]
        turb_imgs = [TF.resize(img[:, rr:rr+hcro, cc:cc+wcro], (ps, ps), antialias=True) for img in turb_imgs]
        tar_img = TF.resize(tar_img[:, rr:rr+hcro, cc:cc+wcro], (ps, ps), antialias=True)
    ...
    def __getitem__(self, index):
        ...
        return torch.stack(turb_imgs, dim=0), torch.stack(other_imgs, dim=0), torch.stack(tar_imgs, dim=0)
        
      
class DataLoaderTurbImageTest(Dataset):
    # equal to previous dynamic
    def __init__(self, rgb_dir, num_frames=48, total_frames=50, im_size=None, noise=None, other_mod='blur', start_frame=0, predix='{:d}.png'):
    ...
    def __getitem__(self, index):
        ...
        return torch.stack(turb_imgs, dim=0), torch.stack(other_imgs, dim=0), tar_imgs, seq_path
